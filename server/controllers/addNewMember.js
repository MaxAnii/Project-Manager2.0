  ...
  try {
    if (type === "College Admin") {
      ...
      data = await pool.query(
        "INSERT INTO department VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *",
        [collegeCode, dname, name, email, password, hodid, id]
      );

      temp = await pool.query(
        'SELECT "collegeName" FROM admin WHERE "collegeCode"=$1',
        [collegeCode]
      );
      ...
      const { id, profid, collegeCode, name, dname, email, password } =
        req.body;

      data = await pool.query(
        "INSERT INTO mentor VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING *",

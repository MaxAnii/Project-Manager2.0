  ...
  const getData = async () => {
    if (projectType === "interDepartment") {
      const mentorResponse = await fetch(
        `http://localhost:5000/getProjectMentorList/${projectType}/${param.collegeCode}/${param.dname}`,
        {
          headers: {
      ...
      const memberResponse = await fetch(
        `http://localhost:5000/getProjectMemberList/${projectType}/${param.collegeCode}/${param.dname}`,
        {
          headers: {
            JToken: localStorage.getItem("JToken"),
          },
      ...
      const mentorResponse = await fetch(
        `http://localhost:5000/getProjectMentorList/${projectType}/${param.collegeCode}`,
        {
          headers: {
            JToken: localStorage.getItem("JToken"),
          },
      ...
      setMentor(mentorData);
      const memberResponse = await fetch(
        `http://localhost:5000/getProjectMemberList/${projectType}/${param.collegeCode}`,
        {
          headers: {
            JToken: localStorage.getItem("JToken"),
          },
    ...
    <>
      ...
      <div className="background">
        <div className="container-content">
          <h3 className="heading">Add new project with Project Harbor</h3>
        </div>
      </div>
      ...
      <div className="project-form">
        <form className="row g-3 FormContainer" onSubmit={submitProject}>
          <div className="col-md-12 ">
            ...
            <div className="form-check form-check-inline">
              <input
                ...
                id="inlineCheckbox2"
                value="interCollege"
                onChange={() => setProjectType("interCollege")}
                name="projectType"
              />
          ...
          <div className="col-12">
            <button key="SubmitForm" type="submit" className="btn btn-success">
              Submit
            </button>
          </div>
        </form>
